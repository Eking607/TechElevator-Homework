...
SELECT film.title, release_year, category.name AS "Category"
FROM film
...
SELECT address.address, address.postal_code, address.district, city.city, country.country
FROM address
...
SELECT customer.first_name, customer.last_name, COUNT(customer.last_name) AS number_of_rentals
FROM rental
...
SELECT customer.first_name, customer.last_name, SUM(payment.amount) AS money_spent
FROM customer
...
SELECT store.store_id, address.address, SUM(payment.amount), AVG(payment.amount), COUNT(inventory.film_id)
FROM store
...
SELECT film.title, COUNT(inventory.film_id) AS number_of_rentals
FROM film
...
SELECT category.name, COUNT(rental.rental_id) AS number_of_rental
FROM film
INNER JOIN inventory ON film.film_id = inventory.film_id
INNER JOIN rental ON inventory.inventory_id = rental.inventory_id
INNER JOIN film_category ON film.film_id = film_category.film_id
...
SELECT actor.first_name, actor.last_name, COUNT(rental.*) AS number_of_rentals_starred_in
FROM actor
...
ORDER BY number_of_rentals_starred_in DESC
LIMIT 10;
...
SELECT actor.first_name, actor.last_name, COUNT(rental.*) AS rental_count
FROM film 
INNER JOIN film_category ON film.film_id = film_category.film_id
INNER JOIN category ON film_category.category_id = category.category_id
INNER JOIN inventory ON film.film_id = inventory.film_id
