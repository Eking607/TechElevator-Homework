...
SELECT name, region FROM country WHERE continent = 'South America' OR continent = 'North America';

-- 12. The name, continent, and head of state of all countries whose form of government is a monarchy (43 rows)
SELECT name, continent, headofstate FROM country WHERE governmentform ILIKE '%Monarchy%';
...
SELECT name, region FROM country WHERE continent = 'South America' or continent = 'North America' AND region != 'Caribbean';
...
SELECT ROUND (gnp*1000000 / population, 2) AS "per capita GNP" FROM country WHERE  continent = 'Europe';
-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
SELECT (2020 - indepyear) AS "number of years since independence" FROM country WHERE indepyear IS NOT NULL